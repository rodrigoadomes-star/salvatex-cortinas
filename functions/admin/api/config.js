...
export async function onRequestGet(context) {
  ...
  const data=await readConfig(context.env.DB,a.storeId);
...
export async function onRequestPut(context) {
  ...
  const current=(await readConfig(context.env.DB,a.storeId)).config;
  const merged=mergeDeep(current,patch);
  const now=new Date().toISOString();
  await context.env.DB.prepare(`INSERT INTO store_configs(store_id,config_key,value_json,updated_at) VALUES(?1,'site_config',?2,?3) ON CONFLICT(store_id,config_key) DO UPDATE SET value_json=excluded.value_json,updated_at=excluded.updated_at`).bind(a.storeId,JSON.stringify(merged),now).run();
  await logAdmin(context.env.DB,"site_config_updated","store_config","site_config",null,a.storeId);
