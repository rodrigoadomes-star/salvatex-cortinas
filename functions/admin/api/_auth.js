...
export async function secureEqual(a,b){return crypto.subtle.timingSafeEqual(await digest(String(a)),await digest(String(b)))}
export async function createAdminSession(secret){const csrf=b64u(crypto.getRandomValues(new Uint8Array(24)));const payload=b64u(encoder.encode(JSON.stringify({iat:Date.now(),exp:Date.now()+SESSION_SECONDS*1000,csrf,nonce:crypto.randomUUID()})));const signature=b64u(new Uint8Array(await crypto.subtle.sign("HMAC",await hmacKey(secret),encoder.encode(payload))));return{token:`${payload}.${signature}`,csrf}}
