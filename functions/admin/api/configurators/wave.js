...
async function readWave(db) {
  ...
  let wave=legacyIntoWave(site, JSON.parse(JSON.stringify(DEFAULT_WAVE)));
...
export async function onRequestPut(context) {
  ...
  try{
    ...
    if(!wave.medidas || Number(wave.medidas.calculoMaximo)<=0){
      return json({ok:false,message:"Informe a altura máxima calculada."},400);
    }

    const now=new Date().toISOString();
    const valueJson=JSON.stringify(wave);
    ...
    console.error("Erro ao salvar configurador Wave:",error);

    return json(
      {
        ok:false,
        message:
          "Erro ao salvar o configurador Wave no banco."
      },
      500
    );
  }
