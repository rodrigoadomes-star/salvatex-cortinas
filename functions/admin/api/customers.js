...
export async function onRequestGet(context){
 ...
 const rows=await context.env.DB.prepare(`WITH emails AS (
   SELECT DISTINCT lower(customer_email) email FROM orders WHERE store_id=?1 AND customer_email IS NOT NULL AND customer_email<>''
 ), order_stats AS (
 ...
 ORDER BY COALESCE(s.spent,0) DESC,COALESCE(a.created_at,s.last_order) DESC LIMIT 500`).bind(storeId).all();
