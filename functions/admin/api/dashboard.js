...
export async function onRequestGet(context) {
  ...
  return json({ ok:true, store:auth.store, stats:{ salesToday:Number(todayRow?.total||0), ordersToday:Number(todayRow?.pedidos||0), averageTicket:ticket, monthRevenue:monthTotal, totalOrders:Number(allRow?.pedidos||0) }, statuses:statuses.results||[], revenue:revenue.results||[], recent:recent.results||[], topProducts:topProducts.results||[] });
}
