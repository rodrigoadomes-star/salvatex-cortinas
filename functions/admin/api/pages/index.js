...
import { ensurePageNavigationSchema,normalizeNavGroup,normalizeExternalUrl } from '../../../api/_page-schema.js';

const PAGE_TYPES=new Set(['conteudo','produtos','link','configurador_wave','configurador_prega_macho','configurador_ilhos','configurador_persiana']);
...
export async function onRequestPost(context){
  ...
  const pageType=normalizePageType(b.pageType);if(pageType.startsWith('configurador_')&&!await configuratorAllowed(context.env.DB,storeId))return json({ok:false,code:'FEATURE_NOT_ENABLED',message:'O configurador não está liberado para esta empresa.'},403);
