...
export async function onRequestGet(context){
  ...
  const rows=await context.env.DB.prepare(`SELECT * FROM pages WHERE store_id=?1 ORDER BY updated_at DESC`).bind(storeId).all();
...
export async function onRequestPost(context){
  ...
  `).bind(
    id,storeId,title,slugify(b.slug||title),sanitizeHtml(b.contentHtml),clean(b.seoTitle,240),
  ...
  await logAdmin(context.env.DB,"page_created","page",id,{title,pageType,productCount:productIds.length,measureCount:measures.length},storeId);
