...
export async function onRequestPut(context){
  ...
  await logAdmin(context.env.DB,"page_updated","page",id,{title,pageType,productCount:productIds.length,measureCount:measures.length},a.storeId);
  return json({ok:true});
}
...
export async function onRequestDelete(context){
  ...
  await context.env.DB.prepare(`DELETE FROM pages WHERE id=?1 AND store_id=?2`).bind(id,a.storeId).run();
  await logAdmin(context.env.DB,"page_deleted","page",id,null,a.storeId);
  return json({ok:true});
}
