...
class ConfiguratorScripts{element(element){element.append('<script src="/js/configurador-media-forro.js?v=20260816-10"></script><script src="/js/configurador-media-strict.js?v=20260816-1"></script>',{html:true});}}
...
async function syncPreviewConfiguratorData(context,url){
  ...
  try{
    ...
    /*
      A cópia de produção serve apenas para inicializar o D1 de Preview.
      Depois disso, nunca sobrescrevemos as alterações feitas no Admin do Preview.
    */
    if(marker?.value_json)return;
    ...
    statements.push(context.env.DB.prepare(`INSERT INTO store_configs(store_id,config_key,value_json,updated_at) VALUES('salvatex',?1,?2,?3) ON CONFLICT(store_id,config_key) DO UPDATE SET value_json=excluded.value_json,updated_at=excluded.updated_at`).bind(markerKey,JSON.stringify({syncedAt:Date.now(),source:PRODUCTION_ORIGIN,mode:"initial-only"}),syncedAt));
    await context.env.DB.batch(statements);
  }catch(error){console.error("preview configurator sync",error);}
}
