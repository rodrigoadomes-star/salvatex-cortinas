...
const PRIVATE_PREFIXES=['/admin','/radz-admin','/login','/api/','/radz/api/'];
function shouldTrack(pathname,response){return !PRIVATE_PREFIXES.some(p=>pathname.startsWith(p))&&(response.headers.get('content-type')||'').includes('text/html')}
export async function onRequest(context){
  const url=new URL(context.request.url),pathname=url.pathname;
  let response=await context.next();
  if(shouldTrack(pathname,response)&&typeof HTMLRewriter!=='undefined'){
    try{response=new HTMLRewriter().on('head',{element(el){el.append('<script src="/js/radz-analytics.js" defer></script>',{html:true})}}).transform(response)}catch{}
  }
  const headers=new Headers(response.headers);
  headers.set("content-security-policy",CSP);headers.set("x-content-type-options","nosniff");headers.set("x-frame-options","DENY");headers.set("referrer-policy","strict-origin-when-cross-origin");headers.set("permissions-policy","camera=(), microphone=(), geolocation=(), payment=()");headers.set("cross-origin-opener-policy","same-origin-allow-popups");headers.set("strict-transport-security","max-age=31536000; includeSubDomains");if(pathname.startsWith("/admin")||pathname.startsWith('/radz-admin'))headers.set("cache-control","no-store");return new Response(response.body,{status:response.status,statusText:response.statusText,headers})
}
