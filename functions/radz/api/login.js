...
async function tooManyAttempts(context, identifierHash, ipHash) {
  try {
    ...
    // Compatibilidade temporária enquanto a migration da Fase 1 não for aplicada.
    return false;
  }
}
...
async function recordAttempt(context, identifierHash, ipHash, success) {
  try {
    ...
    // Não bloqueia o login legado se a migration ainda não foi aplicada.
  }
}
...
export async function onRequestPost(context) {
  ...
  if (!context.env.DB) {
    return json({ ok: false, message: "Banco da plataforma não configurado." }, 503);
  }
  ...
  if (await tooManyAttempts(context, identifierHash, ipHash)) {
    return json({ ok: false, code: "RATE_LIMITED", message: "Muitas tentativas. Aguarde alguns minutos." }, 429);
  }
  ...
  if (email && body.password) {
    const user = await context.env.DB.prepare(`SELECT id,name,email,password_hash,password_salt,password_iterations,role,active
      FROM platform_users
      WHERE email=?1 AND role IN ('platform_owner','platform_support')
      LIMIT 1`)
      .bind(email)
      .first();
    ...
    const auth = { session: { user_id: user.id, role: user.role } };
    await auditRadz(context, auth, "platform.admin.login", "platform_user", user.id, { method: "password" });
    return json({
      ok: true,
  ...
  // Fallback temporário: mantém a chave mestra enquanto usuários individuais são provisionados.
  if (body.token && context.env.RADZ_ADMIN_TOKEN && context.env.RADZ_ADMIN_SESSION_SECRET) {
    ...
    await auditRadz(context, { session: { user_id: null, role: "platform_owner" } }, "platform.admin.login", "legacy_admin", null, { method: "legacy_token" });
    return json({
      ok: true,
      csrfToken: session.csrf,
      expiresIn: 28800,
      user: { id: null, name: "Administrador RADZ HUB (chave mestra)", email: null, role: "platform_owner" },
      legacy: true,
    }, 200, { "set-cookie": sessionCookie(session.token) });
  }
