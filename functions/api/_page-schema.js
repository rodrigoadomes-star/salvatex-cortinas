...
function isProductHub(row){const slug=String(row?.slug||'').trim().toLowerCase();const title=String(row?.title||'').trim().toLowerCase();return['produtos','products','catalogo','catalog'].includes(slug)||['produtos','products','catálogo','catalogo'].includes(title)}
export async function repairLegacyGenericPages(db,storeId){
  ...
  for(const row of rows.results||[]){
    const rawType=String(row.page_type||'conteudo');const legacyId=legacyConfiguratorId(rawType);const normalizedType=normalizePageType(rawType);const normalizedNav=normalizeNavGroup(row.nav_group);const currentCfg=String(row.configurator_id||'');const nextCfg=currentCfg||legacyId;let nextType=normalizedType;
    if(!nextCfg&&nextType==='conteudo'&&isProductHub(row))nextType='produtos';
    if(nextType!==rawType||normalizedNav!==String(row.nav_group||'oculto')||nextCfg!==currentCfg){await db.prepare(`UPDATE pages SET page_type=?1,nav_group=?2,configurator_id=?3 WHERE id=?4 AND store_id=?5`).bind(nextType,normalizedNav,nextCfg||null,row.id,storeId).run()}
