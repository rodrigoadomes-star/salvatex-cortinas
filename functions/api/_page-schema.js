...
function isGenericConfiguratorPage(row){const keys=[row?.slug,row?.title,row?.menu_label].map(key);return keys.some(k=>['sob-medida','sobmedida','personalizado','personalizados','customizado','customizados'].includes(k))}
async function activeConfiguratorIndex(db,storeId){
  const byKey=new Map(),custom=[];
  try{
    ...
    for(const row of rows.results||[]){
      try{
        ...
        if(!['wave','prega-macho','cortina-varao','persiana'].includes(key(id)))custom.push(item);
      }catch{}
    }
  }catch{}
  return{byKey,custom};
...
export async function repairLegacyGenericPages(db,storeId){
  ...
  const cfgIndex=await activeConfiguratorIndex(db,storeId);
  const rows=await db.prepare(`SELECT id,title,slug,menu_label,page_type,nav_group,nav_parent_id,configurator_id,updated_at FROM pages WHERE store_id=?1 ORDER BY updated_at DESC`).bind(storeId).all();
  const normalized=[];
  for(const row of rows.results||[]){
    ...
    const currentCfg=String(row.configurator_id||'').trim();
    let nextCfg=currentCfg||legacyId;
    let nextType=normalizedType;

    if(!nextCfg&&!isProductHub(row)){
      const candidates=[row.slug,row.title,row.menu_label].map(key).filter(Boolean);
      for(const candidate of candidates){if(cfgIndex.byKey.has(candidate)){nextCfg=cfgIndex.byKey.get(candidate);break}}
      if(!nextCfg&&isGenericConfiguratorPage(row)&&cfgIndex.custom.length===1)nextCfg=cfgIndex.custom[0].id;
    }
    if(nextCfg)nextType='configurador';
    else if(nextType==='conteudo'&&isProductHub(row))nextType='produtos';

    if(nextType!==rawType||normalizedNav!==String(row.nav_group||'oculto')||nextCfg!==currentCfg){
      await db.prepare(`UPDATE pages SET page_type=?1,nav_group=?2,configurator_id=?3 WHERE id=?4 AND store_id=?5`).bind(nextType,normalizedNav,nextCfg||null,row.id,storeId).run();
    }
    normalized.push({...row,page_type:nextType,nav_group:normalizedNav,configurator_id:nextCfg});
  }
  const visible=normalized.filter(r=>r.nav_group==='principal');const groups=new Map();for(const row of visible){const label=String(row.menu_label||row.title||'').trim().toLowerCase();const parent=String(row.nav_parent_id||'');if(!label)continue;const groupKey=parent+'|'+label;const arr=groups.get(groupKey)||[];arr.push(row);groups.set(groupKey,arr)}
