...
export async function onRequestGet(context){
  ...
  try{const row=await context.env.DB.prepare(`SELECT value_json FROM store_configs WHERE store_id=?1 AND config_key='site_config'`).bind(tenant.tenant.storeId).first(); if(!row)return json({ok:true,config:null}); let config=null;try{config=JSON.parse(row.value_json)}catch{} return json({ok:true,config});}catch{return json({ok:false,config:null},500)}
}
