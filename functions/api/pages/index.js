...
export async function onRequestGet(context){
  ...
  try{
    ...
    const selected=[q("id"),q("title"),q("slug"),optional("page_type","'conteudo'"),optional("hero_image_url","''"),optional("measures_json","'[]'"),optional("custom_measure_url","''"),optional("nav_group","'oculto'"),optional("nav_order","100"),optional("active","1"),optional("updated_at","NULL")];
    ...
    let stmt=context.env.DB.prepare(sql);if(binds.length)stmt=stmt.bind(...binds);const result=await stmt.all();
    const pages=(result.results||[]).map(row=>{const measures=parseJSON(row.measures_json,[]).map(measure=>({id:String(measure?.id||""),label:String(measure?.label||""),value:String(measure?.value||"")})).filter(measure=>measure.label);return{id:row.id,title:row.title,slug:row.slug,pageType:row.page_type||"conteudo",heroImageUrl:row.hero_image_url||"",navGroup:row.nav_group||"oculto",navOrder:Number(row.nav_order??100),measures,customMeasureUrl:row.custom_measure_url||""}});
