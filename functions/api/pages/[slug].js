...
export async function onRequestGet(context){
  ...
  try{
    ...
    if(page.page_type==='produtos'&&ids.length){const placeholders=ids.map((_,i)=>`?${i+2}`).join(',');const result=await context.env.DB.prepare(`SELECT p.*,c.name category_name,c.slug category_slug FROM products p LEFT JOIN categories c ON c.id=p.category_id AND c.store_id=p.store_id WHERE p.store_id=?1 AND p.active=1 AND p.id IN (${placeholders})`).bind(storeId,...ids).all();const byId=new Map((result.results||[]).map(x=>[x.id,x]));products=ids.map(id=>byId.get(id)).filter(Boolean).map(productPublic)}
    ...
    return json({ok:true,page:{id:page.id,title:page.title,slug:page.slug,pageType:page.page_type||'conteudo',contentHtml:page.content_html||'',seoTitle:page.seo_title||'',seoDescription:page.seo_description||'',heroImageUrl:page.hero_image_url||'',measures,customMeasureUrl:page.custom_measure_url||''},products});
