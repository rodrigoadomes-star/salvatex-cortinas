...
function forcedStorefrontType(page){
  const slug=String(page?.slug||'').trim().toLowerCase();
  const title=String(page?.title||'').trim().toLowerCase();
  if(['produtos','products','catalogo','catalog'].includes(slug)||['produtos','products','catálogo','catalogo'].includes(title))return 'produtos';
  if(['sob-medida','sob_medida','sob-encomenda','sob_encomenda'].includes(slug)||['sob medida','sob encomenda'].includes(title))return 'produtos';
  if(['pronta-entrega','pronta_entrega'].includes(slug)||title==='pronta entrega')return 'produtos';
  return '';
}
async function automaticProducts(db,storeId,page){
  ...
  if(slug==='produtos'||slug==='products'||slug==='catalogo'||slug==='catalog'||title==='produtos'||title==='products'||title==='catálogo'||title==='catalogo'){
    const r=await db.prepare(`${common} ORDER BY p.featured DESC,p.name ASC LIMIT 100`).bind(storeId).all();
    return (r.results||[]).map(productPublic);
  }

  if(slug.includes('sob-medida')||slug.includes('sob_medida')||slug.includes('sob-encomenda')||slug.includes('sob_encomenda')||title.includes('sob medida')||title.includes('sob encomenda')){
    const r=await db.prepare(`${common} AND p.sale_type IN ('sob_medida','sob_encomenda') ORDER BY p.featured DESC,p.name ASC LIMIT 100`).bind(storeId).all();
    return (r.results||[]).map(productPublic);
  }

  if(slug.includes('pronta-entrega')||slug.includes('pronta_entrega')||title.includes('pronta entrega')){
    const r=await db.prepare(`${common} AND p.sale_type='pronta_entrega' ORDER BY p.featured DESC,p.name ASC LIMIT 100`).bind(storeId).all();
    return (r.results||[]).map(productPublic);
  }

  const byCategory=await db.prepare(`${common} AND (LOWER(c.slug)=?2 OR LOWER(c.name)=?3) ORDER BY p.featured DESC,p.name ASC LIMIT 100`).bind(storeId,slug,title).all();
  return (byCategory.results||[]).map(productPublic);
}
export async function onRequestGet(context){
  ...
  try{
    ...
    const rawType=String(page.page_type||'conteudo'),normalizedType=normalizePageType(rawType),pageType=forcedStorefrontType(page)||normalizedType,configuratorId=pageType==='configurador'?String(page.configurator_id||legacyConfiguratorId(rawType)||''):'';
    ...
    if(pageType==='produtos'){
      const forceAutomatic=Boolean(forcedStorefrontType(page));
      if(ids.length&&!forceAutomatic){const placeholders=ids.map((_,i)=>`?${i+2}`).join(',');const result=await context.env.DB.prepare(`SELECT p.*,c.name category_name,c.slug category_slug FROM products p LEFT JOIN categories c ON c.id=p.category_id AND c.store_id=p.store_id WHERE p.store_id=?1 AND p.active=1 AND p.id IN (${placeholders})`).bind(storeId,...ids).all();const byId=new Map((result.results||[]).map(x=>[String(x.id),x]));products=ids.map(id=>byId.get(String(id))).filter(Boolean).map(productPublic)}
