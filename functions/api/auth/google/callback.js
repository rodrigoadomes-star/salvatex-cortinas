...
async function ensureGoogleCustomerSchema(db){
  const cols=await db.prepare('PRAGMA table_info(customer_accounts)').all();
  ...
  await add('google_sub','TEXT');
  await add('picture','TEXT');
  await add('provider',"TEXT DEFAULT 'password'");
  await add('email_verified','INTEGER DEFAULT 0');
  await add('active','INTEGER DEFAULT 1');
  await add('updated_at','TEXT');
  await db.prepare('CREATE UNIQUE INDEX IF NOT EXISTS idx_customer_accounts_google_sub ON customer_accounts(google_sub) WHERE google_sub IS NOT NULL').run();
}
...
export async function onRequestGet(context){
  try{
    ...
    try{
      ...
      const r=await fetch('https://oauth2.googleapis.com/token',{method:'POST',headers:{'content-type':'application/x-www-form-urlencoded'},body});token=await r.json();if(!r.ok||!token.access_token)throw new Error(String(token.error_description||token.error||'token'));
    ...
    console.error('google_oauth_callback_failed',error?.name||'Error',error?.message||String(error));
    return errorPage('O login Google encontrou um erro interno. Tente novamente em alguns instantes.',500);
  }
}
