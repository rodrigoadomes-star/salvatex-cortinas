...
export async function onRequestPost(context){
  ...
  const certs=await fetch('https://www.googleapis.com/oauth2/v3/certs',{cf:{cacheTtl:3600,cacheEverything:true}}).then(r=>r.json());const jwk=(certs.keys||[]).find(k=>k.kid===header.kid&&k.alg==='RS256');if(!jwk)return json({ok:false,message:'Não foi possível validar a assinatura Google.'},401);
  const key=await crypto.subtle.importKey('jwk',jwk,{name:'RSASSA-PKCS1-v1_5',hash:'SHA-256'},false,['verify']);const valid=await crypto.subtle.verify('RSASSA-PKCS1-v1_5',key,base64urlBytes(parts[2]),new TextEncoder().encode(parts[0]+'.'+parts[1]));if(!valid)return json({ok:false,message:'Assinatura Google inválida.'},401);
  const email=String(payload.email||'').toLowerCase(),now=new Date().toISOString();let account=await context.env.DB.prepare('SELECT id FROM customer_accounts WHERE email=?1').bind(email).first();const id=account?.id||crypto.randomUUID();
  ...
  const user={sub:id,userId:id,email,name:String(payload.name||''),picture:String(payload.picture||''),provider:'google',emailVerified:true};const token=await signSession(context,user);return json({ok:true,user},200,{'Set-Cookie':sessionCookie(token)});
}
