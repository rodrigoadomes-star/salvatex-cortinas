...
const titles={dashboard:['Dashboard','Visão geral da sua loja'],orders:['Vendas','Gerencie vendas, pagamentos e envios'],products:['Produtos','Catálogo e produtos da loja'],categories:['Categorias','Organize o catálogo'],customers:['Clientes','Contas cadastradas e compradores'],messages:['Mensagens','Atendimento recebido pelo site'],pages:['Páginas','Conteúdo institucional da loja'],layout:['Layout do site','Cabeçalho, página inicial, cores e rodapé'],media:['Mídia','Imagens usadas no catálogo'],coupons:['Cupons','Descontos e campanhas'],reports:['Relatórios','Desempenho da operação'],configurators:['Configuradores','Produtos sob medida, regras de cálculo e mídia'],settings:['Configurações','Dados gerais da loja'],integrations:['Integrações','Serviços conectados à loja'],billing:['Plano e cobrança','Taxa definida no contrato da RADZ HUB'],logs:['Logs do Sistema','Histórico administrativo']};
...
function ordersTable(rows,compact=false){return `<table class="admin-table orders-list sales-table"><thead><tr><th>Venda</th><th>Data</th><th>Cliente</th><th>Total</th><th>Produtos</th><th>Status</th><th>Ações</th></tr></thead><tbody>${rows.map(o=>`<tr data-order-id="${esc(o.id)}" class="order-row"><td><div class="sale-number-wrap"><b class="sale-number">#${esc(o.sale_number||o.order_number)}</b>${o.internal_notes?`<span class="sale-note" tabindex="0" aria-label="Observação desta venda">▢<span class="sale-note-tip">${esc(o.internal_notes)}</span></span>`:''}</div><small class="order-cell-sub">${esc(o.order_number||'')}</small></td><td>${dateTime(o.created_at)}</td><td>${esc(o.customer_name||o.customer_email||'—')}<small class="order-cell-sub">${esc(o.customer_email||'')}</small></td><td><b>${brlCents(o.total_cents)}</b></td><td><button type="button" class="sale-products-toggle" data-sale-products>${Number(o.item_count||0)} und.⌄</button></td><td><span class="status ${esc(o.status)}">${statusLabel(o.status)}</span></td><td><div class="sale-action-menu"><button type="button" class="sale-more" data-sale-menu="${esc(o.id)}" aria-label="Abrir ações da venda" aria-expanded="false">⋮</button><div class="sale-menu-popover" data-sale-popover="${esc(o.id)}"><button type="button" data-sale-status="em_producao">◇ Marcar em produção</button><button type="button" data-sale-status="pronto">▣ Pronto para envio</button><button type="button" data-sale-label>▤ Imprimir etiqueta Zebra 108×150</button><button type="button" data-sale-status="enviado">▱ Marcar como enviado</button><button type="button" data-sale-edit>✎ Editar venda</button><button type="button" data-sale-status="entregue">✓ Marcar como entregue</button><button type="button" class="sale-cancel" data-sale-status="cancelado">⊘ Cancelar venda</button></div></div></td></tr>`).join('')||'<tr><td colspan="7" class="empty">Nenhuma venda.</td></tr>'}</tbody></table>`}
...
async function renderOrders(){const d=await api('orders');const c=$('#view-content');c.innerHTML=`<div class="page-toolbar"><div class="filters"><input id="order-search" class="search" placeholder="Buscar venda, cliente, e-mail ou telefone"><select id="order-status" class="select"><option value="">Todos os status</option>${['aguardando_pagamento','pago','em_producao','pronto','enviado','entregue','cancelado','reembolsado'].map(s=>`<option value="${s}">${statusLabel(s)}</option>`).join('')}</select><button id="order-filter" class="ghost-btn">Filtrar</button></div><div class="orders-help">Clique na venda para abrir os detalhes ou expanda os produtos diretamente na lista.</div></div><section class="panel sales-panel"><div class="table-wrap sales-table-wrap" id="orders-table">${ordersTable(d.orders)}</div></section>`;bindOrderRows(c);const filtrar=async()=>{const q=encodeURIComponent($('#order-search').value),s=encodeURIComponent($('#order-status').value);$('#orders-table').innerHTML='<div class="empty">Carregando…</div>';const x=await api(`orders?q=${q}&status=${s}`);$('#orders-table').innerHTML=ordersTable(x.orders);bindOrderRows($('#orders-table'))};$('#order-filter').onclick=filtrar;$('#order-search').addEventListener('keydown',e=>{if(e.key==='Enter')filtrar()});$('#order-status').addEventListener('change',filtrar)}
function saleItemsRow(d){const items=(d.items||[]).map(i=>'<div class="sale-product-item"><div class="sale-product-info">'+(i.image?'<img src="'+esc(i.image)+'" alt="">':'<span class="sale-product-placeholder">▧</span>')+'<div><strong>'+esc(i.name)+'</strong><div class="sale-product-details">'+itemDetailText(i)+'</div></div></div><b>'+Number(i.quantity||1)+'</b><span>'+brlCents(i.unit_price_cents)+'</span><strong>'+brlCents(i.total_cents)+'</strong></div>').join('')||'<div class="empty">Nenhum item encontrado.</div>';return '<tr class="sale-products-row"><td colspan="7"><div class="sale-products-head"><b>Produto</b><b>Unidades</b><b>Preço unitário</b><b>Total</b></div><div class="sale-products-list">'+items+'</div></td></tr>'}
function bindOrderRows(root=document){$$('.open-order',root).forEach(b=>b.onclick=e=>{e.stopPropagation();openOrder(b.closest('tr').dataset.orderId)});$$('.order-row',root).forEach(tr=>tr.onclick=e=>{if(!e.target.closest('.sale-action-menu'))openOrder(tr.dataset.orderId)});$$('[data-sale-menu]',root).forEach(btn=>btn.onclick=e=>{e.stopPropagation();const menu=$('[data-sale-popover="'+btn.dataset.saleMenu+'"]',root),open=!menu.classList.contains('open');$$('.sale-menu-popover.open',root).forEach(x=>x.classList.remove('open'));menu.classList.toggle('open',open);btn.setAttribute('aria-expanded',String(open))});$$('[data-sale-status]',root).forEach(btn=>btn.onclick=async e=>{e.stopPropagation();const tr=btn.closest('tr'),status=btn.dataset.saleStatus;if(!confirm('Alterar esta venda para "'+statusLabel(status)+'"?'))return;await api('orders/'+encodeURIComponent(tr.dataset.orderId),{method:'PATCH',body:JSON.stringify({status})});toast('Venda atualizada');await navigate(ADMIN.view,true)});$$('[data-sale-edit]',root).forEach(btn=>btn.onclick=e=>{e.stopPropagation();openOrder(btn.closest('tr').dataset.orderId)});$$('[data-sale-label]',root).forEach(btn=>btn.onclick=async e=>{e.stopPropagation();btn.disabled=true;try{const data=await api('orders/'+encodeURIComponent(btn.closest('tr').dataset.orderId));printHtml(orderPrintHtml(data,true));toast('Etiqueta 108×150 gerada')}catch(err){alert(err.message)}finally{btn.disabled=false}});$$('[data-sale-products]',root).forEach(btn=>btn.onclick=async e=>{e.stopPropagation();const tr=btn.closest('tr'),next=tr.nextElementSibling;btn.dataset.closedLabel=btn.dataset.closedLabel||btn.textContent;if(next?.classList.contains('sale-products-row')){next.remove();btn.textContent=btn.dataset.closedLabel||'Itens⌄';return}btn.disabled=true;try{const data=await api('orders/'+encodeURIComponent(tr.dataset.orderId));tr.insertAdjacentHTML('afterend',saleItemsRow(data));btn.textContent='Ocultar itens⌃'}catch(err){alert(err.message)}finally{btn.disabled=false}})}
