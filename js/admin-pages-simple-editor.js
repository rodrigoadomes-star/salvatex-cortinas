(()=>{
  window.RADZ_MULTI_CONFIGURATOR_PAGES=true;
  ...
  pageForm=async function(x={},products=ADMIN.cache.pageProducts||[],allPages=ADMIN.cache.pages||[]){
    let configurators=[];
    try{const d=await api('configurators');configurators=(d.configurators||[]).filter(c=>c&&c.ativo!==false)}catch{}
    ...
    const linkedConfigurators=Array.isArray(x.configurators)&&x.configurators.length?x.configurators:[cfgId?{configuratorId:cfgId,menuLabel:x.title||'',sortOrder:10,active:true}:null].filter(Boolean);
    const cfgOptions=(selectedId='')=>configurators.map(c=>`<option value="${esc(c.id)}" ${selectedId===c.id?'selected':''}>${esc(c.nome||c.id)}</option>`).join('');
    ...
    openModal(`<h2>${x.id?'Editar página':'Nova página'}</h2><form id="page-form" class="page-simple-editor">
      <div class="page-editor-section">
        ...
        <div class="form-field full" id="page-configurator-wrap"><div class="measure-admin-head"><div><label>Configuradores do submenu</label><small class="field-hint">Selecione os configuradores habilitados desta empresa. Com mais de uma opção, eles aparecem no submenu desta página.</small></div><button type="button" id="add-page-configurator" class="ghost-btn">+ Incluir configurador</button></div><div id="page-configurator-builder" class="page-configurator-builder"></div></div>
    ...
    const f=$('#page-form'),typeEl=$('#page-type'),productsWrap=$('#page-products-wrap'),contentWrap=$('#page-content-wrap'),cfgWrap=$('#page-configurator-wrap'),cfgBuilder=$('#page-configurator-builder'),linkWrap=$('#page-link-wrap'),builder=$('#measure-builder');
    let seq=0;
    ...
    function renderConfigurator(item={}){const row=document.createElement('div');row.className='page-configurator-row';row.innerHTML=`<div class="form-field"><label>Configurador</label><select class="page-configurator-id"><option value="">Selecione...</option>${cfgOptions(item.configuratorId||item.id||'')}</select></div><div class="form-field"><label>Nome mostrado no submenu</label><input class="page-configurator-label" value="${esc(item.menuLabel||item.label||'')}" placeholder="Ex.: Papel Liso"></div><div class="form-field compact"><label>Ordem</label><input class="page-configurator-order" type="number" value="${Number(item.sortOrder||10)}"></div><label class="page-configurator-active"><input class="page-configurator-enabled" type="checkbox" ${item.active===false?'':'checked'}> Ativo</label><button type="button" class="measure-remove danger-link">Remover</button>`;$('.measure-remove',row).onclick=()=>row.remove();cfgBuilder.appendChild(row)}
    linkedConfigurators.forEach(renderConfigurator);$('#add-page-configurator').onclick=()=>renderConfigurator({sortOrder:(cfgBuilder.children.length+1)*10,active:true});
    ...
    f.onsubmit=async e=>{e.preventDefault();const fd=new FormData(f),configuratorItems=$$('.page-configurator-row',cfgBuilder).map((row,index)=>({configuratorId:$('.page-configurator-id',row).value,menuLabel:$('.page-configurator-label',row).value.trim(),sortOrder:Number($('.page-configurator-order',row).value||((index+1)*10)),active:$('.page-configurator-enabled',row).checked})).filter(item=>item.configuratorId);if(fd.get('pageType')==='configurador'&&!configuratorItems.length){alert('Inclua pelo menos um configurador habilitado nesta página.');return}if(new Set(configuratorItems.map(item=>item.configuratorId)).size!==configuratorItems.length){alert('O mesmo configurador não pode ser incluído duas vezes na página.');return}if(configuratorItems.some(item=>!item.menuLabel)){alert('Informe o nome que será mostrado para cada opção do submenu.');return}if(fd.get('pageType')==='link'&&!String(fd.get('externalUrl')||'').trim()){alert('Informe o endereço que este item deve abrir.');return}const measurePayload=$$('.measure-admin-card',builder).map(row=>({id:row.dataset.measureId,label:$('.measure-label',row).value.trim(),value:$('.measure-value',row).value.trim(),productIds:$$('.measure-products input:checked',row).map(i=>i.value)})).filter(m=>m.label);const body={title:fd.get('title'),menuLabel:fd.get('menuLabel'),slug:fd.get('slug'),pageType:fd.get('pageType'),configuratorId:configuratorItems[0]?.configuratorId||'',configurators:configuratorItems,navGroup:fd.get('navGroup'),navParentId:fd.get('navParentId'),navOrder:Number(fd.get('navOrder')||100),externalUrl:fd.get('externalUrl'),heroImageUrl:fd.get('heroImageUrl'),productIds:fd.getAll('productIds'),measures:measurePayload,customMeasureUrl:fd.get('customMeasureUrl'),contentHtml:fd.get('contentHtml'),seoTitle:fd.get('seoTitle'),seoDescription:fd.get('seoDescription'),active:fd.get('active')==='on'};await api(x.id?'pages/'+x.id:'pages',{method:x.id?'PUT':'POST',body:JSON.stringify(body)});toast('Página salva');closeModal();navigate('pages',true)};
  ...
  style.textContent=`.page-simple-editor{display:grid;gap:16px}.page-editor-section{border:1px solid var(--line);border-radius:12px;padding:16px;background:#fff}.page-editor-section h3{margin:0 0 14px;font-size:14px}.page-editor-advanced{border:1px solid var(--line);border-radius:12px;background:#fafbfc}.page-editor-advanced summary{padding:14px 16px;font-weight:700;cursor:pointer}.page-editor-advanced[open] summary{border-bottom:1px solid var(--line)}.page-editor-advanced>.form-grid{padding:16px}.page-simple-editor .form-actions{margin-top:0}.page-configurator-builder{display:grid;gap:10px;margin-top:12px}.page-configurator-row{display:grid;grid-template-columns:minmax(170px,1fr) minmax(180px,1.2fr) 90px auto auto;gap:10px;align-items:end;padding:12px;border:1px solid var(--line);border-radius:10px;background:#fafbfc}.page-configurator-row .form-field{margin:0}.page-configurator-active{padding:10px 0;white-space:nowrap}@media(max-width:820px){.page-configurator-row{grid-template-columns:1fr}.page-configurator-row .measure-remove{justify-self:start}}`;
