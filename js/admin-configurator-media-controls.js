(function(){
  ...
  function enhanceCard(card){
    ...
    if(galleryHidden&&galleryPreview&&!galleryPreview.dataset.deleteReady){
      galleryPreview.dataset.deleteReady="1";
    }
  ...
  document.addEventListener("click",async event=>{
    ...
    try{
      ...
      if(kind==="capa"){
        const hidden=card.querySelector(".cfg-tab-capa");
        if(hidden)hidden.value="";
        const preview=card.querySelector(".cfg-tab-capa-preview");
        if(preview)preview.innerHTML="<span>Sem capa</span>";
      }else if(kind==="video"){
        const hidden=card.querySelector(".cfg-tab-video");
        if(hidden)hidden.value="";
        const preview=card.querySelector(".cfg-tab-video-preview");
        if(preview)preview.innerHTML="<span>Sem vídeo</span>";
        ...
        if(preview){
          preview.innerHTML=urls.length?urls.map(u=>`<span class="cfg-media-thumb-wrap"><img src="${escAttr(u)}"><button type="button" class="cfg-media-thumb-delete" data-media-delete="galeria" data-media-url="${escAttr(u)}" aria-label="Excluir foto">×</button></span>`).join(""):"<span>Sem fotos</span>";
        }
      }
