(()=>{
  ...
  async function mount(){
    if(mounting||!location.hash.includes('layout')||document.getElementById(PANEL_ID)||!document.getElementById('view-content'))return;
    mounting=true;
    try{
      document.querySelectorAll(`#${PANEL_ID}`).forEach((el,i)=>{if(i>0)el.remove()});
      ...
      root.querySelector('#wa-save').onclick=async()=>{
        ...
        try{
          ...
          await req('layout',{method:'PUT',body:JSON.stringify({layout:next})});feedback.style.color='#16794b';feedback.textContent='Configuração do WhatsApp salva para todo o site.';
    ...
    }catch(error){console.warn('[RADZ admin WhatsApp]',error)}finally{mounting=false}
  }
  style();
