(()=>{
  const PANEL_ID='radz-whatsapp-admin-panel';
  const STYLE_ID='radz-whatsapp-admin-style';
  const INSTANCE_KEY='__radzWhatsappAdminSingleton';

  if(window[INSTANCE_KEY]?.observer){
    try{window[INSTANCE_KEY].observer.disconnect()}catch(_){ }
  }
  window[INSTANCE_KEY]={observer:null,timer:null,mounting:false};
  const state=window[INSTANCE_KEY];
  ...
  async function req(path,options={}){
    ...
    if(!['GET','HEAD','OPTIONS'].includes(method)){
      ...
      const token=csrf();if(token)headers['x-csrf-token']=token;
    }
  ...
  function removeDuplicates(){
    const nodes=[...document.querySelectorAll('[data-radz-whatsapp-admin],#'+PANEL_ID)];
  ...
  async function mount(){
    ...
    const onLayout=location.hash.includes('layout');
    const container=document.getElementById('view-content');
    if(!onLayout||!container)return;

    const existing=removeDuplicates();
    if(existing&&container.contains(existing))return;
    if(existing&&!container.contains(existing))existing.remove();

    state.mounting=true;
    try{
      const d=await req('layout');
      if(!location.hash.includes('layout'))return;
      const target=document.getElementById('view-content');
      if(!target)return;

      document.querySelectorAll('[data-radz-whatsapp-admin],#'+PANEL_ID).forEach(el=>el.remove());

      const layout=d.layout&&typeof d.layout==='object'?d.layout:{};
      const wa=layout?.footer?.whatsapp||layout?.whatsapp||{};
      const root=document.createElement('section');
      root.id=PANEL_ID;
      root.dataset.radzWhatsappAdmin='1';
      root.className='panel';
      root.innerHTML=`<div class="panel-head"><div><h2>WhatsApp flutuante</h2><p class="wa-admin-help">Uma única configuração para todas as páginas públicas desta loja.</p></div><div class="wa-preview">${icon()}</div></div><div class="wa-admin-switch"><div><strong>Mostrar botão do WhatsApp</strong><div class="wa-admin-help">Aparece automaticamente em todas as páginas públicas da loja.</div></div><input id="wa-enabled" type="checkbox" ${wa.enabled!==false?'checked':''}></div><div class="wa-admin-grid" style="margin-top:16px"><div class="wa-admin-field"><label for="wa-number">Número do WhatsApp</label><input id="wa-number" inputmode="tel" placeholder="5544999999999" value="${String(wa.number||'').replace(/"/g,'&quot;')}"><span class="wa-admin-help">Use DDI + DDD + número. Ex.: 5544991234567.</span></div><div class="wa-admin-field"><label for="wa-position">Posição do botão</label><select id="wa-position"><option value="right" ${wa.position!=='left'?'selected':''}>Canto inferior direito</option><option value="left" ${wa.position==='left'?'selected':''}>Canto inferior esquerdo</option></select></div><div class="wa-admin-field full"><label for="wa-message">Mensagem inicial</label><textarea id="wa-message" placeholder="Olá! Vim pelo site e gostaria de mais informações.">${String(wa.message||'').replace(/[&<>]/g,c=>({'&':'&amp;','<':'&lt;','>':'&gt;'}[c]))}</textarea></div></div><div id="wa-feedback" class="wa-admin-help" style="margin-top:10px"></div><div class="wa-admin-actions"><button type="button" class="primary-btn" id="wa-save">Salvar WhatsApp</button></div>`;
      target.appendChild(root);

      root.querySelector('#wa-save').onclick=async()=>{
        const button=root.querySelector('#wa-save'),feedback=root.querySelector('#wa-feedback');
        button.disabled=true;feedback.textContent='Salvando…';
        try{
          const latest=await req('layout');
          const next=latest.layout&&typeof latest.layout==='object'?structuredClone(latest.layout):{};
          next.footer=next.footer&&typeof next.footer==='object'?next.footer:{};
          next.footer.whatsapp={
            enabled:root.querySelector('#wa-enabled').checked,
            number:root.querySelector('#wa-number').value.replace(/\D/g,''),
            message:root.querySelector('#wa-message').value.trim(),
            position:root.querySelector('#wa-position').value==='left'?'left':'right'
          };
          delete next.whatsapp;
          await req('layout',{method:'PUT',body:JSON.stringify({layout:next})});
          feedback.style.color='#16794b';feedback.textContent='Configuração do WhatsApp salva para todo o site.';
        }catch(error){
          feedback.style.color='#b42318';feedback.textContent=error.message||'Não foi possível salvar.';
        }finally{button.disabled=false}
      };
    }catch(error){
      console.warn('[RADZ admin WhatsApp]',error);
    }finally{
  ...
  function schedule(){
    ...
    state.timer=setTimeout(mount,120);
  }

  ensureStyle();
  document.querySelectorAll('[data-radz-whatsapp-admin],#'+PANEL_ID).forEach(el=>el.remove());
  state.observer=new MutationObserver(schedule);
  state.observer.observe(document.getElementById('admin-app')||document.body,{childList:true,subtree:true});
