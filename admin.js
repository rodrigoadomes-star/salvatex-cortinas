const ADMIN={csrf:sessionStorage.getItem('radzHubAdminCsrf')||'',company:{id:'salvatex',name:'Salvatex Cortinas',cnpj:''},view:'dashboard',cache:{},currency:new Intl.NumberFormat('pt-BR',{style:'currency',currency:'BRL'})};
...
async function api(path,options={}){const method=(options.method||'GET').toUpperCase();const headers={'x-radzhub-company-id':ADMIN.company.id,...(options.headers||{})};if(!(options.body instanceof FormData))headers['content-type']='application/json';if(!['GET','HEAD','OPTIONS'].includes(method)&&ADMIN.csrf)headers['x-csrf-token']=ADMIN.csrf;const r=await fetch('/admin/api/'+path,{...options,credentials:'same-origin',headers});let d={};try{d=await r.json()}catch{}if(r.status===401){sessionStorage.removeItem('radzHubAdminCsrf');ADMIN.csrf='';throw new Error('Sessão inválida ou expirada')}if(!r.ok)throw new Error(d.message||'Erro ao acessar o servidor');return d}
async function login(token){try{const r=await fetch('/admin/api/login',{method:'POST',credentials:'same-origin',headers:{'content-type':'application/json'},body:JSON.stringify({token,companyId:ADMIN.company.id})});const d=await r.json();if(!r.ok)throw new Error(d.message||'Credencial inválida');ADMIN.csrf=d.csrfToken;sessionStorage.setItem('radzHubAdminCsrf',ADMIN.csrf);$('#admin-token-input').value='';$('#admin-login').style.display='none';$('#admin-app').hidden=false;syncCompanyContext();await navigate(location.hash.slice(1)||'dashboard')}catch(e){ADMIN.csrf='';sessionStorage.removeItem('radzHubAdminCsrf');$('#login-error').textContent=e.message}}
async function logout(){try{if(ADMIN.csrf)await api('logout',{method:'POST',body:'{}'})}catch{}sessionStorage.removeItem('radzHubAdminCsrf');ADMIN.csrf='';location.reload()}
...
function syncCompanyContext(){const name=$('#active-company-name'),cnpj=$('#active-company-cnpj');if(name)name.textContent=ADMIN.company.name;if(cnpj)cnpj.textContent=ADMIN.company.cnpj?'CNPJ: '+ADMIN.company.cnpj:'CNPJ: configurar'}
const titles={dashboard:['Dashboard','Visão geral da empresa ativa'],companies:['Empresas','Cadastre e identifique cada operação por empresa e CNPJ'],orders:['Pedidos','Gerencie pedidos e andamento'],products:['Produtos','Catálogo e produtos da loja'],categories:['Categorias','Organize o catálogo'],customers:['Clientes','Base formada pelos pedidos'],pages:['Páginas','Conteúdo institucional da loja'],media:['Mídia','Imagens usadas no catálogo'],coupons:['Cupons','Descontos e campanhas'],reports:['Relatórios','Desempenho da operação'],configurators:['Configuradores','Produtos sob medida, regras de cálculo e mídia'],settings:['Configurações','Dados gerais da loja'],integrations:['Integrações','Serviços conectados à loja'],billing:['Plano e cobrança','Taxa definida no contrato da RADZ HUB'],logs:['Logs do Sistema','Histórico administrativo']};
async function navigate(view,force=false){ADMIN.view=titles[view]?view:'dashboard';$$('#admin-nav button').forEach(b=>b.classList.toggle('active',b.dataset.view===ADMIN.view));$('#view-title').textContent=titles[ADMIN.view][0];$('#view-subtitle').textContent=titles[ADMIN.view][1];$('.sidebar').classList.remove('open');const c=$('#view-content');c.innerHTML='<div class="empty">Carregando…</div>';try{await ({dashboard:renderDashboard,companies:renderCompanies,orders:renderOrders,products:renderProducts,categories:renderCategories,customers:renderCustomers,pages:renderPages,media:renderMedia,coupons:renderCoupons,reports:renderReports,configurators:renderConfigurators,settings:renderSettings,integrations:renderIntegrations,billing:renderBilling,logs:renderLogs}[ADMIN.view])()}catch(e){c.innerHTML='<div class="panel empty">'+esc(e.message)+'</div>'}}
...
async function renderCompanies(){
  let d;try{d=await api('companies')}catch(e){$('#view-content').innerHTML='<section class="panel empty"><b>Estrutura multiempresa preparada.</b><br>'+esc(e.message)+'</section>';return}
  const companies=d.companies||[];
  $('#view-content').innerHTML=`<div class="page-toolbar"><div><h2 style="margin:0">Empresas</h2><small style="color:var(--muted)">Empresa e CNPJ são obrigatórios para configurar uma operação.</small></div><button id="new-company" class="primary-btn">Cadastrar empresa</button></div><section class="panel"><div class="table-wrap"><table class="admin-table"><thead><tr><th>Empresa</th><th>Razão social</th><th>CNPJ</th><th>Status</th></tr></thead><tbody>${companies.map(x=>`<tr><td><strong>${esc(x.tradeName)}</strong><span class="order-cell-sub">${esc(x.slug)}</span></td><td>${esc(x.legalName)}</td><td>${esc(String(x.cnpj).replace(/^(\\d{2})(\\d{3})(\\d{3})(\\d{4})(\\d{2})$/,'$1.$2.$3/$4-$5'))}</td><td><span class="status ${x.status==='active'?'pago':'cancelado'}">${x.status==='active'?'Ativa':'Inativa'}</span></td></tr>`).join('')||'<tr><td colspan="4" class="empty">Nenhuma empresa cadastrada.</td></tr>'}</tbody></table></div></section>`;
  $('#new-company').onclick=()=>{openModal(`<h2>Cadastrar empresa</h2><form id="company-form"><div class="form-grid"><div class="form-field full"><label>Nome da empresa</label><input name="tradeName" maxlength="120" required></div><div class="form-field full"><label>Razão social</label><input name="legalName" maxlength="180" required></div><div class="form-field"><label>CNPJ</label><input name="cnpj" inputmode="numeric" maxlength="18" required></div><div class="form-field"><label>Identificador</label><input name="slug" maxlength="80" placeholder="gerado pelo nome"></div></div><div class="form-actions"><button type="button" class="ghost-btn" data-close-modal>Cancelar</button><button class="primary-btn">Cadastrar empresa</button></div></form>`);$('#company-form').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.currentTarget);await api('companies',{method:'POST',body:JSON.stringify({tradeName:fd.get('tradeName'),legalName:fd.get('legalName'),cnpj:fd.get('cnpj'),slug:fd.get('slug')})});toast('Empresa cadastrada');closeModal();renderCompanies()}};
}
