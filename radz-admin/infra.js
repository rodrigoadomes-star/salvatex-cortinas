...
async function load(){
 ...
 try{
  ...
  content.innerHTML=`<div class="cards"><article class="config-card"><h2>Runtime principal</h2><p><span class="status ${d.ready?"active":"trial"}">${d.ready?"Pronto para unificação":"Configuração incompleta"}</span></p><p>A RADZ HUB é o runtime principal. Esta verificação mostra apenas presença de recursos e compatibilidade do schema; nenhum valor secreto é exibido.</p>${d.missing?.length?`<p class="form-result"><strong>Faltando:</strong><br>${d.missing.map(esc).join('<br>')}</p>`:'<p class="form-result"><strong>Ambiente completo para os itens monitorados.</strong></p>'}</article><article class="config-card"><h2>Bindings</h2><div class="table-wrap"><table><thead><tr><th>Binding</th><th>Status</th></tr></thead><tbody>${rows(d.checks?.bindings)}</tbody></table></div></article></div><div class="cards"><article class="config-card"><h2>RADZ HUB</h2><div class="table-wrap"><table><thead><tr><th>Variável</th><th>Status</th></tr></thead><tbody>${rows(d.checks?.radz)}</tbody></table></div></article><article class="config-card"><h2>Compatibilidade Salvatex</h2><p>Esses itens são temporariamente necessários enquanto a loja antiga é absorvida pela plataforma principal.</p><div class="table-wrap"><table><thead><tr><th>Variável</th><th>Status</th></tr></thead><tbody>${rows(d.checks?.salvatex_legacy_runtime)}</tbody></table></div></article></div><div class="cards"><article class="config-card"><h2>D1 — estruturas essenciais</h2><div class="table-wrap"><table><thead><tr><th>Estrutura</th><th>Status</th></tr></thead><tbody>${rows(d.checks?.d1)}</tbody></table></div></article><article class="config-card"><h2>Compatibilidade da tabela pages</h2><p>${d.schema?.pages?.exists?'Tabela base encontrada.':'Tabela base ausente ou incompleta.'}</p>${optionalMissing.length?`<p class="form-result"><strong>Colunas opcionais ainda ausentes:</strong><br>${optionalMissing.map(esc).join('<br>')}<br><br>O endpoint público usa fallback compatível e não deve mais ficar preso em “Carregando opções…”.</p>`:'<p class="form-result"><strong>Schema de páginas completo.</strong></p>'}</article></div>`;
